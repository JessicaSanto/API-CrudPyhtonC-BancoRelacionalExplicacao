...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:senai%40134@127.0.0.1/ProjetoCD'
...
# como JSON (JavaScript Object Notation) ou XML (eXtensible Markup Language), que pode ser enviado através de uma rede.
    def to_json(self):
        return{"id":self.id, "marca": self.marca, "modelo": self.modelo, "ano": self.ano}
...
# 'request.get_json()' converte o corpo da requisição em um dicionário Python.
    ...
    try:
        carro = Carros(marca=body["marca"], modelo= body["modelo"], ano= body["ano"])
